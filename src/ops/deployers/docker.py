...
class DockerDeployer(BaseDeployer):
    def deploy(
        ...
        proxmox.exec(
            vmid,
            f"cd {app_dir} && docker compose down >/dev/null 2>&1 || true",
            node=node,
        )
        build_flag = " --build" if docker_cfg.build_context else ""
        proxmox.exec(
            vmid, f"cd {app_dir} && docker compose up -d{build_flag}", node=node
        )
    ...
    def get_logs(
        ...
        result = proxmox.exec(
            vmid,
            f"cd {app_dir} && docker compose logs{flag} {docker_cfg.service_name}",
            node=node,
        )
    ...
    def restart_service(
        ...
        proxmox.exec(
            vmid,
            f"cd {app_dir} && docker compose restart {docker_cfg.service_name}",
            node=node,
        )
    ...
    def get_service_status(
        ...
        result = proxmox.exec(
            vmid,
            f"cd {app_dir} && docker compose ps {docker_cfg.service_name}",
            node=node,
        )
